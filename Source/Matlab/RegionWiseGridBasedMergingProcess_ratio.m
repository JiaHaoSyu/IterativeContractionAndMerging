function [Label] = RegionWiseGridBasedMergingProcess_ratio(OperationImg,XY,RatioSpatial,BinColor,InputM)
% Syntax:
%   [Label] = RegionWiseGridBasedMergingProcess_ratio(OperationImg,XY,RatioSpatial,BinColor,InputM)
% Description:
...
%   XY : Twisted spatial coordinate with Kx2 matrix
%   RatioSpatial : discretization invervals of x,y 
%   BinColor : discretization invervals of L,a,b
%   ColorI : Twisted L,a,b with Kx3 matrix 

% Output arguments:
%   Label : Output the segments after grid-based merging process with Kx1
%   matrix
...
InputMMin = min(InputM,[],1);
for k = 1:Dim
    OperationImgMin(k) = min(min(min(OperationImg(:,:,k))),InputMMin(k));
end

OperationImgMax = zeros(Dim,1);
InputMMax = max(InputM,[],1);
for k = 1:Dim
    OperationImgMax(k) = max(max(max(OperationImg(:,:,k))),InputMMax(k));
end
ColorBinRange = (OperationImgMax-OperationImgMin)/ BinColor;
% Spacial information for grid-based method
% BinSpatialRange = min( I_Width, I_Height ) / BinSpatial;
...
dz1 = zeros(size(InputM));

for i = 1:Dim
    dz1(:,i) = (( InputM(:,i)-OperationImgMin(i)) / ColorBinRange(i) ) + 0.5;
end

% (L*_bin)x(a*_bin)x(b*_bin)x(x_bin)x(y_bin)
...
tempInd = ImageToGrid_Index;
gridWeightsC = sparse(tempInd(:), 1, ones(numel(tempInd),1) , gridNodeNumber,1);

%% non_empty_OperationImg construction
[NonEmptyGridIndexC, ~, ~] = find(gridWeightsC);

NonEmptyGridIndex = NonEmptyGridIndexC;

non_empty_num = numel(NonEmptyGridIndex);

% Compute the mapping vector from cell index to Re-labeling index
grid2NonemptyIndex = SparseMatrix(NonEmptyGridIndex,ones(size(NonEmptyGridIndex,1),1),1:non_empty_num,size(NonEmptyGridIndex,1),gridNodeNumber,1);
